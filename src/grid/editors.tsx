            ...
            set({ data, focusArea }) {
              ...
              */
              if (parentNode?.direction === 'col') {
                let newParentNode = generateRow([cloneNode, newNode]);
                let index = parentNode?.children.findIndex((item) => item.id === id);
            ...
            set({ data, focusArea, slots }) {
              ...
              */
              if (parentNode.children.length === 2) {
                let anthorNode = parentNode.children.find((item) => item.id !== id);
                let grandNode = findParentNode(parentNode.id, data.tree);

                if (grandNode) {
                  let index = grandNode.children.findIndex((item) => item.id === parentNode.id);
