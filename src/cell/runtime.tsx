...
export default function ({ env, data, slots, inputs, outputs }) {
  ...
  return (
    <View className={css.cell} onClick={() => { onClick({ title: data.title }) }}>

      {data.useThumb && data.thumb ? (
        <Image className={cx(css.thumb, "mybricks-thumb")} mode="scaleToFill" src={data.thumb} />
      ) : null}
      ...
      <View className={cx(css.inner)}>
        <View className={css.label}>
          ...
          {data.brief ? (
            <View className={cx(css.brief, "mybricks-brief")}>{data.brief}</View>
          ) : null}
        </View>

        <View className={cx(css.content, {
          ...
          {data.useArrowIcon ? (
            <View className={css.arrow}>
              <Arrow className={css.icon} style={data.useChildren && data.useArrowIcon ? { color: data.arrowIconColor } : {}} />
            </View>
          ) : null}
