...
export default {
  ":root"(props) {
    const { input, output, inputs, outputs } = props;
    const inputSchemaArray = input.get().map(({ id }) => {
      const schema = inputs.get(id).schema;
      return {
        id: id.split(".")[1],
        schema: JSON.stringify(
          unknownType.includes(schema.type) ? { type: anyType } : schema
        ),
      };
    });
    ...
    outputSchemaArray.forEach(({ id, schema }) => {
      outputsSchemaStr =
        outputsSchemaStr + `${id}函数输出的值的JSON Schema定义为${schema},`;
    });
